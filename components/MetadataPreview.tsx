...
export default function MetadataPreview({ design }: MetadataPreviewProps) {
  const [copied, setCopied] = useState(false);

  // Match the exact format that will be generated and uploaded
  const logoUrl = design.logoUrl || (design.logoCid ? `${process.env.NEXT_PUBLIC_IPFS_GATEWAY || 'https://ipfs.pruuf.tech/ipfs/'}${design.logoCid}` : '');
  const backgroundUrl = design.backgroundUrl || (design.backgroundCid ? `${process.env.NEXT_PUBLIC_IPFS_GATEWAY || 'https://ipfs.pruuf.tech/ipfs/'}${design.backgroundCid}` : '');
